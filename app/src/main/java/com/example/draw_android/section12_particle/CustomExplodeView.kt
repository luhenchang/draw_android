    ...
    private fun updateAllExplodeParticle() {
        particleList.forEach {
            ...
            if (s>it.recycleDistance) {
                ...
                it.color = Color.argb(255,Random().nextInt(255),Random().nextInt(255),Random().nextInt(255))
            }else{
                it.pointX = s * cos(it.angle)
                it.pointY = s * sin(it.angle)
                it.velocity +=it.acceleration
                it.color = Color.argb((255*(1-s/it.recycleDistance)).toInt(),Random().nextInt(255),Random().nextInt(255),Random().nextInt(255))
            }
        }
    }
